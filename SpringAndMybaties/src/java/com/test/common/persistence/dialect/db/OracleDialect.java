    ...
    public String insert(Class<?> clazz, Object param) throws Exception{
        ...
        if(sqlCache.containsKey(sqlCacheKey)){
            ...
            if (fields != null && fields.length > 0) {
                        ...
                        if(isBatchSave){
                            switch (field.getType().getName()) {
                                ...
                                case "java.lang.Short":
                                    paramSql.append("#{" + entityPre + "." + field.getName() + ", jdbcType=SMALLINT} AS " + fieldColumn + COLUMN_SPLIT);
                                    break;

                                case "int":
                                case "java.lang.Integer":
                                    paramSql.append("#{" + entityPre + "." + field.getName() + ", jdbcType=INTEGER} AS " + fieldColumn + COLUMN_SPLIT);
                                    break;

                                case "long":
                                case "java.lang.Long":
                                    paramSql.append("#{" + entityPre + "." + field.getName() + ", jdbcType=BIGINT} AS " + fieldColumn + COLUMN_SPLIT);
                                    break;

                                case "java.math.BigDecimal":
                                    paramSql.append("#{" + entityPre + "." + field.getName() + ", jdbcType=NUMERIC} AS " + fieldColumn + COLUMN_SPLIT);
                                    break;

                                case "java.lang.String":
                                    paramSql.append("#{" + entityPre + "." + field.getName() + ", jdbcType=VARCHAR} AS " + fieldColumn + COLUMN_SPLIT);
                                    break;

                                case "java.util.Date":
                                case "java.sql.Date":
                                case "java.sql.Timestamp":
                                    paramSql.append("#{" + entityPre + "." + field.getName() + ", jdbcType=TIMESTAMP} AS " + fieldColumn + COLUMN_SPLIT);
                                    break;

                                case "java.sql.Clob":
                                    paramSql.append("#{" + entityPre + "." + field.getName() + ", jdbcType=CLOB} AS " + fieldColumn + COLUMN_SPLIT);
                                    break;

                                case "java.sql.Blob":
                                    paramSql.append("#{" + entityPre + "." + field.getName() + ", jdbcType=BLOB} AS " + fieldColumn + COLUMN_SPLIT);
                                    break;
                ...
                if(isBatchSave){
                    sql.append(Constants.CrudDaoSql.FOR_EACH_SAVE_FLAG)
                    .append(" SELECT ")
            ...
            if (obj instanceof List) {
                ...
                if (baseEntities != null && baseEntities.size() > 0) {
                    for (int i=0;i<baseEntities.size();i++){
                        ...
                        }else if(StringUtils.equalsIgnoreCase("long", idField.getType().getSimpleName()) && (idObj==null || StringUtils.equalsIgnoreCase("0",String.valueOf(idObj)))){
                ...
                for(Field field : fields){
                    if(field.isAnnotationPresent(SQLHelper.P_ID)){
                        ...
                        if (objValue != null) {
                            ...
                            Object updateFields = ReflectionsUtils.getFieldValue(((MapperMethod.ParamMap) param).get("entity"), "updateFields");
                            if(updateFields!=null&&updateFields instanceof Set &&((Set)updateFields).contains(field.getName().toLowerCase())){
                                String fieldColumn = SQLHelper.getFieldColumn(field);
                                setSql.append(fieldColumn + " = null " + COLUMN_SPLIT);
                            }
                        }
                    }
                }
...
//                commonSqlSessionTemplate.insert("com.asiainfo.esop.common.persistence.dao.CommonMethodMapper." + Constants.CrudDaoSql.INSERT_UPB_HIS, map);
...
//                map.put(Constants.CrudDaoSql.KEY_DAO_CLASS,clazz);
//                map.put(Constants.CrudDaoSql.KEY_PARAMS,param);
//                commonSqlSessionTemplate.insert("com.asiainfo.esop.common.persistence.dao.CommonMethodMapper." + Constants.CrudDaoSql.INSERT_DEL_HIS, map);
//            }
//        }
    }
...
//        }
    ...
    private void dealFiledColumn(Object baseEntity,Object columnObj,String simpleTableName,Field field,String fieldColumn,String entityName,StringBuilder whereSql,boolean likeFlag,boolean likeEntity){
        ...
        } else if (columnObj instanceof Number) {
            if(likeEntity){
                whereSql.append(" AND " + simpleTableName + "." + fieldColumn + "= #{" + entityName + "." + fieldName + "}");
            }else{
                whereSql.append(" AND " + simpleTableName + "." + fieldColumn + " != #{" + entityName + "." + fieldName + "}");
