 ...
 */
package com.test.common.persistence.dialect.db;


import com.test.common.persistence.annotation.Entity;
...
import com.test.common.persistence.entity.Rank;
    ...
    public String findById(Class<?> clazz, Object param) throws Exception{
        ...
        if(sqlCache.containsKey(sqlCacheKey)){
            ...
            sql.append(columnSql.substring(0,columnSql.length()-1))
                    .append(" FROM " + SQLHelper.getTableName(clazz) +" AS " + simpleTableName + tableSql)
                    .append(whereSql);
            ...
            if (obj instanceof Array) {
                ...
                if (baseEntities != null && baseEntities.length > 0) {
                    for (BaseEntity baseEntity : baseEntities) {
                        ...
                        if(idObj instanceof String && StringUtils.isEmpty(String.valueOf(idObj))){
                            ReflectionsUtils.setFieldValue(baseEntity,idField,baseEntity.newStringId());
                        }else if(idObj instanceof Number && StringUtils.equalsIgnoreCase("0",String.valueOf(idObj))){
            ...
            if(fields!=null && fields.length>0){
                ...
                for(Field field : fields){
                    if(field.isAnnotationPresent(SQLHelper.P_ID)){
                        ...
                        Object objValue = ReflectionsUtils.getFieldValue(param,field);
                        if(objValue!=null){
                            String fieldColumn = SQLHelper.getFieldColumn(field);
...
//            Field idField = SQLHelper.getIdField(clazz);
//            if(idField!=null) {
//                if (hisTableConfigEntity != null && hisTableConfigEntity.getDeleteType() == 1) {//当删除的时候更新表状态
//                    String columnName = hisTableConfigEntity.getColumnName();
//                    String deleteFlag = hisTableConfigEntity.getDeleteFlag();
//                    resultSql = "UPDATE " + tableName + " SET " + columnName + "='" + deleteFlag + "' WHERE " + SQLHelper.getFieldColumn(idField) + "=#{entity." + idField.getName() + "}";
//                }else {
//                    resultSql = "DELETE FROM " + tableName + " WHERE " + SQLHelper.getFieldColumn(idField) + "=#{entity." + idField.getName() + "}";
//                }
//            }else{
...
//                    String columnName = hisTableConfigEntity.getColumnName();
//                    String deleteFlag = hisTableConfigEntity.getDeleteFlag();
//                    sb.append("UPDATE " + tableName + " SET " + columnName + "='" + deleteFlag);
//                }else{
...
//            }
    ...
    public String insertUpbHis(Class<?> clazz,Object param) throws Exception{
        ...
        Field idField = SQLHelper.getIdField(clazz);
        if(idField!=null){
            if(param !=null && (param instanceof Map) && ((Map) param).containsKey("id")){
                whereSql.append(" WHERE " + SQLHelper.getFieldColumn(idField) + "=#{id})");
            }else{
                whereSql.append(" WHERE " + SQLHelper.getFieldColumn(idField) + "=#{entity."+idField.getName()+"})");
            }
...
//                Map map = new HashMap();
//                map.put(Constants.CrudDaoSql.KEY_DAO_CLASS,clazz);
//                map.put(Constants.CrudDaoSql.KEY_PARAMS,param);
//                sqlSessionFactory.openSession().insert("com.asiainfo.esop.common.persistence.dao.CommonMethodMapper." + Constants.CrudDaoSql.INSERT_UPB_HIS, map);
...
//                map.put(Constants.CrudDaoSql.KEY_DAO_CLASS,clazz);
//                map.put(Constants.CrudDaoSql.KEY_PARAMS,param);
//                sqlSessionFactory.openSession().insert("com.asiainfo.esop.common.persistence.dao.CommonMethodMapper." + Constants.CrudDaoSql.INSERT_DEL_HIS, map);
//            }
//        }
    }
    ...
    private String getHisSql(Class<?> clazz,Object param,String hisType) throws Exception{
        ...
        String simpleTableName = SQLHelper.getSimpleTableName(clazz);
        String tableName = SQLHelper.getTableName(clazz);
//        CfgHisTableConfigEntity hisTableConfigEntity = HisEntityFactory.getHisTableConfigEntity(tableName);
//        if(hisTableConfigEntity!=null){
//            String hisTableName = "";
//            if(StringUtils.equals("UPB",hisType)){//修改
...
//                throw new Exception(hisTableConfigEntity.getTableName()+" no found hisTable config");
//            }
//
...
//                sql.append(" (SELECT "+newId+",a.*,sysdate FROM "+ SplitTableFactory.getSplitTable(tableName,param) +" a");
//            }else{
//                sql.append(" ( SELECT a.*,sysdate FROM "+SplitTableFactory.getSplitTable(tableName,param) + " a");
//            }
//        }
        return sql.toString();
    }
                ...
                if(field.isAnnotationPresent(SQLHelper.P_ENTITY)){
                    ...
                    if(pEntity.autoLoad()) {
                        ...
                        if(pEntity.cond()!=null && pEntity.cond().length>0){
                            ...
                            tempChildSql.append(") AS " + childSimpleTableName);
                        }else{
                            tempChildSql.append(childTableName + " AS " + childSimpleTableName);
                        }
                    ...
                    String fieldColumn = SQLHelper.getFieldColumn(field);
                    if (StringUtils.isNotEmpty(fieldColumn)){
                        columnSql.append(simpleTableName+"."+fieldColumn + " AS " + (SQLHelper.getSimpleClassName(clazz.getSimpleName())+"_"+field.getName()).toLowerCase() + COLUMN_SPLIT);
                    }
                }
            }
    ...
    private void dealFiledColumn(Object baseEntity,Object columnObj,String simpleTableName,Field field,String fieldColumn,String entityName,StringBuilder whereSql,boolean likeFlag,boolean likeEntity){
        ...
        } else if (columnObj instanceof Number && !StringUtils.equals("0", String.valueOf(columnObj))) {
            if(likeEntity){
                whereSql.append(" AND " + simpleTableName + "." + fieldColumn + "= #{" + entityName + "." + fieldName + "}");
            }else{
                whereSql.append(" AND " + simpleTableName + "." + fieldColumn + " != #{" + entityName + "." + fieldName + "}");
            ...
            if(lists.size()==1){
                ...
                if(likeEntity) {
                    for (int i = 0; i < lists.size(); i++) {
                        ...
                        if (columnObj != null) {
                            if ((columnObj instanceof String && StringUtils.isEmpty(columnObj.toString())) || (columnObj instanceof Number && StringUtils.equalsIgnoreCase("0", columnObj.toString().trim()))) {
                                continue;
                            }
